...
class Rect:
    ...
    def intersect(self, other):
        ...
        return self.x1 <= other.x2 and self.x2 >= other.x1 and self.y1 \
            <= other.y2 and self.y2 >= other.y1
...
class Map:
    ...
    def generate_map(self):
        ...
        self.tiles = [[Tile(True) for y in range(MAP_HEIGHT)] for x in
                      range(MAP_WIDTH)]
...
def menu(
    ...
    libtcod.console_blit(  # opacity of fg, bg = 1.0, 1.0
        window,
        0,
        0,
        width,
...
npc_fighter_component = Fighter(hp=30, defense=2, power=5,
                                death_function=monster_death)
npc = Object(
    0,
    0,
...
for y in range(map.h):
    for x in range(map.w):
        libtcod.map_set_properties(fov_map, x, y, not map.at(x,
                                   y).block_sight, not map.at(x,
                                   y).blocked)
...
message('Welcome! Don\'t lose your shoes because its a long walk through the DUNGEON OF HARD STONE FLOORS!'
        , libtcod.red)
